    ...
    const $this = {
        ...
        splash: async () => {
            ...
            try {
                ...
                $('head title').html($this.options.windowTitle)

                $this.world = new (await $this.requireClass('World'))();
        ...
        requireClass: async function(_className) {
            if (!$this.classes) {
                $this.classes = {}
            }

            await waitFor(() => {
                return $this.classes[_className]
            })
            return $this.classes[_className]
        },
        ...
        gfxSetup: () => {
            createCanvas($('body').innerWidth(), $('body').innerHeight());
        },
        gfxDraw: () => {
            background(0);
        },
        gfxWindowResized: () => {
            waitForFinalEvent(() => {
                resizeCanvas($('body').innerWidth(), $('body').innerHeight());
                log('Resized canvas')
            }, 500, 'resizewindow')
        }
    };
